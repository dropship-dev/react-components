...
function RangeDatePicker(props) {
    ...
    function convertDate(date, type) {
        ...
        console.log(`${date.getFullYear()}-${date.getMonth() + 1}-${date.getDate()} ${type === "start" ? "00:00:00" : "23:59:59"}${(0, moment_timezone_1.default)(date).tz(timezoneDate).format("Z")}`);
        return new Date(`${date.getFullYear()}-${date.getMonth() + 1}-${date.getDate()} ${type === "start" ? "00:00:00" : "23:59:59"}${(0, moment_timezone_1.default)(date).tz(timezoneDate).format("Z")}`);
    }
